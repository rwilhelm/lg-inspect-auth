...
(function() {
    ...
    var config = {
        api: {
            trips: 'trips',
            check: function(id) {
                return 'trips/' + id + '/check';
            },
            count: function(id) {
                return 'trips/' + id + '/count';
            },
            sensors: function(id, sensor) {
                return 'trips/' + id + '/' + sensor;
            }
        },
        windowSize: 200
    };
    ...
    // AUTH COPY PASTE BEGIN
    // .config(['$routeProvider', function($routeProvider) {

    //     $routeProvider
    //         .when('/login', {
    //             controller: 'LoginController',
    //             templateUrl: 'login.html', // <-- XXX
    //             hideMenus: true
    //         })

    //     .when('/', {
    //         controller: 'AppCtrl',
    //         templateUrl: 'index.html' // <-- XXX
    //     })

    //     .otherwise({
    //         redirectTo: '/login' // <-- XXX
    //     });
    // }])
    ...
    .controller('appCtrl', ['$rootScope', '$scope', '$location', '$route', '$routeParams', 'Data',
        function($rootScope, $scope, $location, $route, $routeParams, Data) {
            ...
            $scope.deleteTrip = function(id) {
                $scope.trips = $scope.trips.filter(function(trip) {
                    return trip.id !== id;
                });
                Data.deleteTrip(id);
            };
    ...
    .factory('Data', ['$http', '$q', function($http, $q) {
        return {
            ...
            loadData: function(trip, params, more) {
                ...
                var sensors = _.keys(trip.count).filter(function(d) {
                    return trip.count[d];
                });
